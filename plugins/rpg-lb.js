...
let text = `
╭───═[ *Top ${len} ${settings.moneda_rpg}* ]═────⋆
│╭───────────────···
✩│ Tú eres el *${usersLim.indexOf(m.sender) + 1}* de *${usersLim.length}*
✩│ ${sortedLim.slice(0, len).map(({ jid, limit }, i) => `${i + 1}. ${participants.some(p => jid === p.jid) ? `(${conn.getName(jid)}) wa.me/` : '@'}${jid.split`@`[0]} *${limit} ${settings.moneda_rpg}*`).join`\n✩│ `}
│╰────────────────···
╰───────────═┅═──────────
